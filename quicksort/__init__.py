...
class QuickSort:
    ...
    def partition(self, data, head, tail, draw_data, time_tick):
        ...
        draw_data(data, self.get_color_array(len(data), head, tail, border, border))
        time.sleep(time_tick)

        for j in range(head, tail):
            if data[j] < pivot:
                draw_data(data, self.get_color_array(len(data), head, tail, border, j, True))
                time.sleep(time_tick)
        ...
        draw_data(data, self.get_color_array(len(data), head, tail, border, tail, True))
        time.sleep(time_tick)
